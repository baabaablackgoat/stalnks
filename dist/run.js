...
const embedColors_1 = require("./classes/embedColors");
const getEnv_1 = require("./functions/getEnv");
const Client_1 = require("./functions/Client");
const UserEntry_1 = require("./classes/UserEntry");
// ensure data exists
...
function updatePricesDatabase(userId) {
    ...
    if (err) {
        ...
        try {
            ...
            for (const key in rawData) {
                try {
                    userData[key] = UserEntry_1.UserEntry.fromRaw(key, rawData[key]);
                }
                catch (entryErr) {
...
function bestStonksEmbed() {
    ...
    output.color = embedColors_1.default.Default;
...
Client_1.default.on('message', msg => {
    ...
    if (msg.content.startsWith(msgPrefix + helpInvoker)) {
        const helpEmbed = new Discord.MessageEmbed({
            ...
            color: embedColors_1.default.Default,
            fields: [
                {
                    name: "Register",
        ...
        if (msg.mentions.members.size > 0) {
            if (msg.mentions.members.size > 1) {
                const moreThanOneProfileEmbed = new Discord.MessageEmbed({
                    ...
                    color: embedColors_1.default.Warning,
    ...
    if (msg.content.startsWith(msgPrefix + timezoneInvoker)) {
        ...
        if (timezone == 'list') {
            const timezoneListEmbed = new Discord.MessageEmbed({
                author: { name: msg.member.displayName, iconURL: msg.author.avatarURL() },
                color: embedColors_1.default.Info,
        ...
        if (userData.hasOwnProperty(msg.author.id))
            ...
            userData[msg.author.id] = new UserEntry_1.UserEntry(msg.author.id, timezone, null, null, null, null, true);
        if (inaccurateTimezones.includes(timezone)) {
            const confirmDangerousTimezoneSetEmbed = new Discord.MessageEmbed({
                author: { name: msg.member.displayName, iconURL: msg.author.avatarURL() },
                color: embedColors_1.default.Ok,
    ...
    if (msg.content.startsWith(msgPrefix + fcInvoker)) {
        ...
        if (['remove', 'delete', 'no'].includes(fc)) {
            ...
            const friendcodeRemovedEmbed = new Discord.MessageEmbed({
                author: { name: msg.member.displayName, iconURL: msg.author.avatarURL() },
                color: embedColors_1.default.Error,
        ...
        if (userData.hasOwnProperty(msg.author.id)) {
            ...
            userData[msg.author.id] = new UserEntry_1.UserEntry(msg.author.id, null, fc, null, null, null, true);
    ...
    if (msg.content.startsWith(msgPrefix + queueInvoker)) {
        ...
        msg.author.send({ embed: {
            ...
            dodoCodeCollector.on('end', (collected, reason) => {
                if (reason == 'time' || collected.size != 1) {
                    ...
                    if (queueData.hasOwnProperty(msg.author.id))
                        queueData[msg.author.id].joinReactionCollector.stop("User failed to create queue in time.");
            ...
            informationEmbed.color = embedColors_1.default.Default;
